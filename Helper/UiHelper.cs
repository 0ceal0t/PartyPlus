...
namespace PartyPlus.Helper {
    public static unsafe partial class UiHelper {
        ...
        public static void SetScale(AtkResNode* atkUnitBase, float? scaleX, float? scaleY) {
            if (scaleX >= short.MinValue && scaleX <= short.MaxValue) atkUnitBase->ScaleX = (short)scaleX.Value;
            if (scaleY >= short.MinValue && scaleY <= short.MaxValue) atkUnitBase->ScaleY = (short)scaleY.Value;
        }
